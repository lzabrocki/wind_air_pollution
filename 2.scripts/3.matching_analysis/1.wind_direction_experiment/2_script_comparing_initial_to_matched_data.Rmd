...
# load matching data
data_matching <-
  readRDS(here::here("1.data", "6.matched_data", "matching_data.Rds")) %>%
...
```{r, fig.width=15, fig.height=5, dev = "CairoPNG"}
...
data_categorical_weather_variables <- data_all %>%
  ...
  mutate(
    rainfall_duration = case_when(
      ...
      rainfall_duration == "Rainfall Duration: [  1,  14)" ~ "Rainfall Duration: [1, 14) min",
      rainfall_duration == "Rainfall Duration: [ 14, 158)" ~ "Rainfall Duration: [14, 158) min",
      rainfall_duration == "Rainfall Duration: [158,1440]" ~ "Rainfall Duration: [158, 1440] min"
    )
  ) %>%
  mutate(
...
graph_categorical_weather_variables <-
  ...
  scale_fill_manual(values = c(my_blue, my_orange),
                    guide = guide_legend(reverse = TRUE)) +
  facet_wrap( ~ values, scale = "free_x", ncol = 4) +
  ylab("") +
  xlab("Proportion (%)") +
  labs(fill = "Dataset:") +
  theme_tufte() +
...
```{r, fig.width=20, fig.height=8, dev = "CairoPNG"}
...
```{r, fig.width=15, fig.height=6, dev = "CairoPNG"}

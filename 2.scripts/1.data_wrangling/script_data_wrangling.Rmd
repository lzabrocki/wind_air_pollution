...
dates_data <- dates_data %>%
  mutate(weekend = ifelse(weekday %in% c("Saturday", "Sunday"), "Weekend", "Work Days"))
````
...
data_not_imputed <- data %>%
  ...
  select(date, mean_no2:mean_pm25) %>%
  rename_at(vars(mean_no2:mean_pm25), ~ paste("not_imputed", ., sep = "_")) %>%
  ungroup()
